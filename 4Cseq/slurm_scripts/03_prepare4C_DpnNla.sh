...
pathWithInstall="$PWD/"
pathWithTableWithGenomes="${gitHubDirectory}/4Cseq/table.txt"
pathForScripts="${gitHubDirectory}/scripts/"
# Not used here: pathWithBRFiles=
pathWithtemplate4cfile="${gitHubDirectory}/4Cseq/template_4cfile.fa"
...
for ((i=1;i<=${n};i++)); do
  ...
  if [ -e $PWD/res_files_mapping_${genome} ]; then
    ...
    if [ ! -e ${pathForScripts}/build_config_4Cseq_5Mb_and_primerfile_withReplicates_individual_merge_DpnNla.sh ]; then
      echo "${pathForScripts}/build_config_4Cseq_5Mb_and_primerfile_withReplicates_individual_merge_DpnNla.sh does not exists"
      exit 1
    fi    
    bash ${pathForScripts}/build_config_4Cseq_5Mb_and_primerfile_withReplicates_individual_merge_DpnNla.sh $PWD ${genome} ${assembly} ${pathWithMutantGenome} template_4cfile_${genome}.fa
  fi
done
