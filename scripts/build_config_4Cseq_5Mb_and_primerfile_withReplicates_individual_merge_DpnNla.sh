...
for f in ${resultDir}*/*.bam ; do
  ...
  # If it is the first replicate we need to put a group which will be the merged:
  if [ "$groupName" != "$lastgroupname" ]; then
    if [ $groupInChunk -ge $maxChunk ]; then
    ...
    # If there is more than one replicate we update the groupName
    if [ $n -gt 1 ]; then
      groupName="${groupName}_neq${n}"
    fi
    echo "[[${lastj}]]" >> config_4cseq_${genome}_groups.txt
    ...
    echo "library_file_url='${pathWithMutantGenome}/${genome}/library_${genome}_DpnNla_30bps_segmentInfos.bed' #set the path the the lib" >> config_4cseq_${genome}_groups.txt
  ...
  if [ $n -gt 1 ]; then
    # For each sample we write the individual group and run:
    let groupInChunk++
    echo "[[${j}]]" >> config_4cseq_${genome}_groups.txt
    echo "name='${sample}'" >> config_4cseq_${genome}_groups.txt
    echo "library_file_type_id=''" >> config_4cseq_${genome}_groups.txt
    echo "library_id=''" >> config_4cseq_${genome}_groups.txt
    echo "library_file_url='${pathWithMutantGenome}/${genome}/library_${genome}_DpnNla_30bps_segmentInfos.bed' #set the path the the lib" >> config_4cseq_${genome}_groups.txt
    echo "library_param_file=''" >> config_4cseq_${genome}_groups.txt
    ...
    echo "" >> config_4cseq_${genome}_groups.txt
    # We add the group to the primer file.
    # We assume that the name of the viewpoint is part of the sample name separated by _ and that the viewpoint is part of the template.
    cat ${templatePrimerFile} | awk -F "|" -v gn=${sample} -v OFS="|" '
      BEGIN{
        split(gn, gns, "_")
        for (i in gns) keywords[gns[i]] = ""
        written = 0
    ...
    echo "group_id=${j}" >> config_4cseq_${genome}_runs.txt
    let j++
  fi
  # Now we add the sample run to the group with pooled replicates
  echo "[[${j}]]" >> config_4cseq_${genome}_runs.txt
  echo "url='${f}'" >> config_4cseq_${genome}_runs.txt
  wig=`ls ${resultDir}*/${sample}_merged.sql`
  echo "url_wig='${wig}'" >> config_4cseq_${genome}_runs.txt
